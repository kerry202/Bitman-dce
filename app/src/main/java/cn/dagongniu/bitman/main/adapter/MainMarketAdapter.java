    ...
    protected void convert(BaseViewHolder helper, IndexPageBean.DataBean.RecommendMarketListBean data1) {
        if (lineDataRefresh == null) {
            ...
            if (Double.parseDouble(lineData.get(helper.getLayoutPosition()).getMarketCoin().getIncRate()) > 0) {
                helper.setTextColor(R.id.tv_left_coin_price, ContextCompat.getColor(context, R.color.bule));
            ...
            //涨跌幅 百分比
            if (Double.parseDouble(lineData.get(helper.getLayoutPosition()).getMarketCoin().getIncRate()) > 0) {
                helper.setTextColor(R.id.tv_applies, ContextCompat.getColor(context, R.color.bule));
            ...
            //人民币价格
            try {
                if (data1.getMarketCoin().getCoinId() == 25) {
                    //人民币价格
                    //人民币换算
            ...
            switch (bigDecimaLast.compareTo(bigDecimaNew)) {
                ...
                case 0://等于 时，返回 0
                    ...
                    break;
                case -1://小于 时，返回 -1
                    helper.setTextColor(R.id.tv_left_coin_price, ContextCompat.getColor(mContext, R.color.bule));
                    break;
            }
            ...
            try {
                if (data1.getMarketCoin().getCoinId() == 25) {
                    //人民币价格
                    //人民币换算
                    BigDecimal cny = lineData.get(helper.getLayoutPosition()).getMarketCoin().getCnyPrice().multiply(new BigDecimal(lineDataRefresh.get(helper.getLayoutPosition()).getMarketCoin().getLastTradePrice()))
                            .setScale(4, BigDecimal.ROUND_DOWN);
                    helper.setText(R.id.tv_cny_price, "≈¥" + cny + "");
                } else {
                    //人民币换算

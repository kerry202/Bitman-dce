...
public class BGABannerUtil {
    ...
    public static Bitmap getScaledImageFromResource(@NonNull Context context, int resId, int maxWidth, int maxHeight, float minWidth, float minHeight) {
        BGABannerUtil.LoadBitmapPair<Throwable> result;
        do {
     ...
     */
    public static BGABannerUtil.LoadBitmapPair<Throwable> getImageFromResource(@NonNull Context context, int resId, int maxWidth, int maxHeight) {
        BGABannerUtil.LoadBitmapPair<Throwable> result;
        ...
        try {
            if (maxWidth == 0 && maxHeight == 0) {
                ...
                result = new BGABannerUtil.LoadBitmapPair<>(BitmapFactory.decodeStream(is, null, decodeOptions), null);
                is.close();
            } else {
                ...
                if (tempBitmap != null && (tempBitmap.getWidth() > desiredWidth || tempBitmap.getHeight() > desiredHeight)) {
                    result = new BGABannerUtil.LoadBitmapPair<>(Bitmap.createScaledBitmap(tempBitmap, desiredWidth, desiredHeight, true), null);
                    tempBitmap.recycle();
                } else {
                    result = new BGABannerUtil.LoadBitmapPair<>(tempBitmap, null);
                }
            }
        } catch (OutOfMemoryError exception) {
            exception.printStackTrace();
            result = new BGABannerUtil.LoadBitmapPair(null, exception);

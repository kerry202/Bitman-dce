    ...
    public void update(String des) {
        ...
        switch (checkBy) {
            ...
            case CHECK_BY_VERSION_NAME:
                ...
                toUpdate(des);
//                String s1 = serverVersionName.replace(".", "");
//                String s2 = localVersionName.replace(".", "");
//                int i1 = Integer.parseInt(s1);
//                int i2 = Integer.parseInt(s2);
//                if (i1 > i2) {
//                    Log.i(TAG, "当前版本是最新版本:" + s1 + "/" + s2);
//
//                } else {
//                    Log.i(TAG, "当前版本是最新版本" + serverVersionCode + "/" + serverVersionName);
//                }
    ...
    private void toUpdate(String des) {
        realUpdate(des);
    }

    private void realUpdate(final String des) {
        ConfirmDialog dialog = new ConfirmDialog(activity, des, new Callback() {
            @Override
...
//                                DownloadAppUtils.downloadForAutoInstall(activity, apkPath, "demo.apk", serverVersionName);
                                ...
                                new ConfirmDialog(activity, des, new Callback() {
                                    @Override
                                    public void callback(int position) {
                                        if (position == 1) {
                                            DownloadAppUtils.download(activity, apkPath, serverVersionName, true);
